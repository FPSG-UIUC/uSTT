...
# Description: A parameterized STT Age Matrix rename circuit. 
# input:
...
# input [YROT_WIDTH:0] yrot_table [NUM_ARCH-1:0],
...
# output [YROT_WIDTH:0] younger_outputs [NUM_DECODE-1:0]
...
file = open(f'stt_{NUM_DECODE}_{YROT_WIDTH}.input', 'w')
...
def stt_logic(instrs_list, branch_ptr, last_epoch, yrot_table):
    ...
    for instr in instrs_list:
        ...
        # update yrot_table
        if load_bit:
            # if load instr, yrot(dest) = last_epoch
            yrot_table_tmp[dest] = last_epoch_tmp
        elif branch_bit:
            # if branch instr, update last_epoch
            last_epoch_tmp = new_branch_ptr[1]
    ...
    return ouput_yrots, younger_outputs, new_branch_ptr, last_epoch_tmp
...
def yrot_msb(yrot, branch_head, branch_tail, wrap_bit, wrap_flag):
    # check if wrap around
    if yrot < branch_head:
        if wrap_flag:
            # branch tail has wrap around
            # wrap_bit is up-to-date
            if wrap_bit:
                return "1"
            else:
                return "0"
        else:
            # branch tail has no wrap around
            if wrap_bit:
                return "0"
            else:
                return "1"
    else:
        # yrot is not wrap around
        if wrap_flag:
            # branch tail has wrap around
            if wrap_bit:
                return "0"
...
for _ in range(NUM_TESTS):
    ...
    output_yrots, younger_outputs, new_branch_ptr, last_epoch_tmp = stt_logic(instrs_list, [branch_head, branch_tail], last_epoch, yrot_table)
    yrot_table_updated = update_yrot_table(yrot_table, instrs_list, output_yrots, write_en)
    
    
    if DEBUG:
        print("[+] Input")
        print(f"Branch head: {branch_head}, Branch tail: {branch_tail}, Last epoch: [{yrot_msb(last_epoch, branch_head, branch_tail, wrap_bit, wrap_flag)}]{last_epoch}, wrap_bit: {wrap_bit}")
        print("input insts:")
        for inst_idx in range(NUM_DECODE):
            rand_instr_type, dest, src1, src2 = instrs_list[inst_idx]
            print(f"({bin(rand_instr_type,2)}) dest: {dest}->{rat_list[dest]}([{yrot_msb(yrot_table[dest], branch_head, branch_tail, wrap_bit, wrap_flag)}]{yrot_table[dest]}), ",
                  f"src1: {src1}->{rat_list[src1]}([{yrot_msb(yrot_table[src1], branch_head, branch_tail, wrap_bit, wrap_flag)}]{yrot_table[src1]}), src2: {src2}->{rat_list[src2]}([{yrot_msb(yrot_table[src2], branch_head, branch_tail, wrap_bit, wrap_flag)}]{yrot_table[src2]})")
        print("phy reg pool:")
        for inst_idx in range(NUM_DECODE):
            print(phyreg_list[inst_idx])
        print("[+] Output:")
        print(f"New Branch head: {new_branch_ptr[0]}, New Branch tail: {new_branch_ptr[1]}, New Last epoch: [{yrot_msb(last_epoch_tmp, branch_head, branch_tail, wrap_bit, wrap_flag)}]{last_epoch_tmp}")
        print(f"write_en: {write_en}")
        print("out_insts:")
        for inst_idx in range(NUM_DECODE):
            p_dest, p_src1, p_src2 = out_insts[inst_idx]
            print(f"dest: {p_dest}([{yrot_msb(output_yrots[inst_idx], branch_head, branch_tail, wrap_bit, wrap_flag)}]{output_yrots[inst_idx]}), src1: {p_src1}, src2: {p_src2}, yrot: [{yrot_msb(younger_outputs[inst_idx], branch_head, branch_tail, wrap_bit, wrap_flag)}]{younger_outputs[inst_idx]}")

    # test vector
    # [rat[0],...,rat[NUM_ARCH-1],phyreg[0],...,phyreg[NUM_DECODE-1],
    # instr[0],...,instr[NUM_DECODE-1], out_insts[0],...,out_insts[NUM_DECODE-1],
    # yrot_table[0],...,yrot_table[NUM_ARCH-1], last_epoch, branch_tail, wrap_bit,
    # output_yrots[0],...,output_yrots[NUM_DECODE-1],younger_outputs[0],...,younger_outputs[NUM_DECODE-1]]
    rat_bitstr = []
    ...
    for yrot_idx in range(NUM_ARCH):
        yrot_table_bitstr.append(yrot_msb(yrot_table[yrot_idx], branch_head, branch_tail, wrap_bit, wrap_flag))
        yrot_table_bitstr.append(bin(yrot_table[yrot_idx], width=YROT_WIDTH))
    yrot_table_bitstr = "".join(yrot_table_bitstr)
    last_epoch_bitstr = []
    last_epoch_bitstr.append(yrot_msb(last_epoch, branch_head, branch_tail, wrap_bit, wrap_flag))
    last_epoch_bitstr.append(bin(last_epoch, width=YROT_WIDTH))
    last_epoch_bitstr = "".join(last_epoch_bitstr)
    branch_tail_bitstr = bin(branch_tail, width=YROT_WIDTH)
    wrap_bit_str = bin(wrap_bit, width=1)
    younger_outputs_bitstr = []
    for yrot_idx in range(NUM_DECODE):
        younger_outputs_bitstr.append(yrot_msb(last_younger_output[yrot_idx], branch_head, branch_tail, wrap_bit, wrap_flag))
    ...
    file.write("".join([rat_bitstr, phyreg_bitstr, instrs_bitstr, 
                        out_insts_bitstr, yrot_table_bitstr, last_epoch_bitstr, branch_tail_bitstr, wrap_bit_str,
                        younger_outputs_bitstr]) + '\n')
    ...
    if (branch_tail < branch_head) and (branch_tail != 0):
        wrap_bit = 1
        wrap_flag = 1
    last_epoch = last_epoch_tmp
